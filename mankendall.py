...
# CSVファイルを読み込む
data = pd.read_csv(csv_file, header=None).iloc[:, 3]
...
plt.ylabel('Memory usage [GB]')  # y軸ラベル
...
plt.gca().yaxis.set_major_formatter(FuncFormatter(format_gb))
plt.yticks(np.arange(4 * 1024, 16 * 1024 + 1, 2 * 1024))
